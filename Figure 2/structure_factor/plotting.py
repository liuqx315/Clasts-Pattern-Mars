...
def plot_summary(
    ...
    **binning_params
    ...
    # print(f"Saved: {file_name}_binned_statistics.csv")
    axis.errorbar(
        bin_centers,
        bin_mean,
        yerr=2 * bin_std,  # 3 times the standard deviation
        fmt=fmt,
        lw=1,
        ecolor=ecolor,
        capsize=2,
    ...
    axis.legend(loc=loc_position, framealpha=0.2)
...
def plot_estimation_showcase(
    ...
    **binning_params
    ...
    if error_bar:
        plot_summary(k_norm, estimation, axis=axis, scale=scale, file_name=file_name, **binning_params)
    ...
    if file_name:
        fig = axis.get_figure()
        fig.savefig(f"{file_name}_Sk.pdf", bbox_inches="tight", dpi=600)
    return axis
...
def plot_estimation_imshow(k_norm, si, axis, file_name):
    ...
    if len(k_norm.shape) < 2:
        ...
        plt.colorbar(f_0, ax=axis, fraction=0.046,pad=0.04)
...
def plot_estimation_all(
    ...
    **binning_params
    ...
    if file_name:
        figure.savefig(f"{file_name}_Sk.pdf", bbox_inches="tight",dpi=600)
